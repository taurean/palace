...
var require_lib = __commonJS({
    "node_modules/obsidian-dataview/lib/index.js"(exports) {
        ...
        var IANAZone = class extends Zone {
            ...
            equals(otherZone) {
                return (
                    otherZone.type === "iana" && otherZone.name === this.name
                );
            }
            /** @override **/
            get isValid() {
        ...
        var Locale = class {
            ...
            static create(
                ...
                const numberingSystemR =
                    numberingSystem || Settings2.defaultNumberingSystem;
                const outputCalendarR =
                    outputCalendar || Settings2.defaultOutputCalendar;
                return new Locale(
                    localeR,
                    numberingSystemR,
                    outputCalendarR,
            ...
            constructor(locale, numbering, outputCalendar, specifiedLocale) {
                ...
                this.numberingSystem =
                    numbering || parsedNumberingSystem || null;
                this.outputCalendar =
        ...
        function untruncateYear(year) {
            if (year > 99) {
                ...
                return year > Settings2.twoDigitCutoffYear
                    ? 1900 + year
                    : 2e3 + year;
        }
        ...
        var Formatter = class {
            ...
            formatDateTimeFromString(dt, fmt) {
                const knownEnglish = this.loc.listingMode() === "en",
                    ...
                    maybeMacro = (token) => {
                        const formatOpts =
                            Formatter.macroTokenToFormatOpts(token);
                        if (formatOpts) {
                    ...
                    tokenToString = (token) => {
                        switch (token) {
                            ...
                            case "uu":
                                return this.num(
                                    Math.floor(dt.millisecond / 10),
                                    2,
                                );
                            case "uuu":
                                return this.num(
                                    Math.floor(dt.millisecond / 100),
                                );
                            case "m":
        ...
        var isoTimeBaseRegex =
            /(\d\d)(?::?(\d\d)(?::?(\d\d)(?:[.,](\d{1,30}))?)?)?/;
        var isoTimeRegex = RegExp(
            `${isoTimeBaseRegex.source}${isoExtendedZone}`,
        );
        var isoTimeExtensionRegex = RegExp(`(?:T${isoTimeRegex.source})?`);
        var isoYmdRegex = /([+-]\d{6}|\d{4})(?:-?(\d\d)(?:-?(\d\d))?)?/;
        var isoWeekRegex = /(\d{4})-?W(\d\d)(?:-?(\d))?/;
        var isoOrdinalRegex = /(\d{4})-?(\d{3})/;
        var extractISOWeekData = simpleParse(
            "weekYear",
            "weekNumber",
            "weekDay",
        );
        ...
        function extractIANAZone(match2, cursor) {
            const zone = match2[cursor]
                ? IANAZone.create(match2[cursor])
                : null;
        ...
        function extractISODuration(match2) {
            ...
            const maybeNegate = (num, force = false) =>
                num !== void 0 && (force || (num && hasNegativePrefix))
                    ? -num
                    : num;
            return [
                {
        ...
        var Duration = class {
            ...
            static fromISO(text2, opts) {
                ...
                if (parsed) {
                    ...
                    return Duration.invalid(
                        "unparsable",
                        `the input "${text2}" can't be parsed as ISO 8601`,
                    );
                }
            }
            /**
            ...
            toFormat(fmt, opts = {}) {
                ...
                return this.isValid
                    ? Formatter.create(
                          this.loc,
                          fmtOpts,
                      ).formatDurationFromString(this, fmt)
                    : INVALID$2;
            }
            /**
            ...
            toISO() {
                ...
                if (this.seconds !== 0 || this.milliseconds !== 0)
                    s2 +=
                        roundTo(this.seconds + this.milliseconds / 1e3, 3) +
                        "S";
                if (s2 === "P") s2 += "T0S";
                return s2;
            }
                ...
                for (const k of orderedUnits$1) {
                    if (units.indexOf(k) >= 0) {
                        ...
                        for (const down in vals) {
                            if (
                                orderedUnits$1.indexOf(down) >
                                orderedUnits$1.indexOf(k)
                            ) {
        ...
        var Info = class {
            ...
            static months(
                ...
                return (
                    locObj ||
                    Locale.create(locale, numberingSystem, outputCalendar)
                ).months(length);
            }
            /**
        ...
        function digitRegex({ numberingSystem }, append2 = "") {
            return new RegExp(
                `${numberingSystems[numberingSystem || "latn"]}${append2}`,
            );
        }
            ...
            const one = digitRegex(loc),
                ...
                unitate = (t) => {
                    ...
                    switch (t.val) {
                        ...
                        case "EEE":
                            return oneOf(
                                loc.weekdays("short", false, false),
                                1,
                            );
                        case "EEEE":
        ...
        function parseDataToDateTime(
            ...
            if (parsed && Object.keys(parsed).length !== 0) {
                ...
                return DateTime.invalid(
                    new Invalid(
                        "unparsable",
                        `the input "${text2}" can't be parsed as ${format}`,
                    ),
                );
            }
        }
        ...
        function quickDT(obj, opts) {
            const zone = normalizeZone(opts.zone, Settings2.defaultZone),
                loc = Locale.fromObject(opts),
                tsNow = Settings2.now();
            let ts, o;
        ...
        var DateTime = class {
            ...
            static fromObject(obj, opts = {}) {
                ...
                const tsNow = Settings2.now(),
                    ...
                    definiteWeekDef =
                        normalized.weekYear || normalized.weekNumber,
                    loc = Locale.fromObject(opts);
                ...
                const higherOrderInvalid = useWeekData
                    ...
                    invalid =
                        higherOrderInvalid || hasInvalidTimeData(normalized);
                if (invalid) {
                    return DateTime.invalid(invalid);
                }
            ...
            static fromFormat(text2, fmt, opts = {}) {
                ...
                const { locale = null, numberingSystem = null } = opts,
                    ...
                    [vals, parsedZone, specificOffset, invalid] =
                        parseFromTokens(localeToUse, text2, fmt);
                if (invalid) {
                    return DateTime.invalid(invalid);
                } else {
                    return parseDataToDateTime(
            ...
            get weekdayShort() {
                return this.isValid
                    ? Info.weekdays("short", { locObj: this.loc })[
                          this.weekday - 1
                      ]
                    : null;
            }
            ...
            get weekdayLong() {
                return this.isValid
                    ? Info.weekdays("long", { locObj: this.loc })[
                          this.weekday - 1
                      ]
                    : null;
            }
       ...
       */
            ...
            toRelative(options = {}) {
                ...
                const base =
                        options.base ||
                        DateTime.fromObject({}, { zone: this.zone }),
            ...
            !(function (n2, t) {
                ...
                return (function (n2) {
                    ...
                    function r(e) {
                        ...
                        return (
                            n2[e].call(u.exports, u, u.exports, r),
                            (u.l = true),
                            u.exports
                        );
                    }
                    ...
                    function (n2, t, r) {
                        ...
                        function p(n3, t2) {
                            if (
                                ...
                                throw new Error(
                                    n3 +
                                        " requires integer length in range [0, 6].",
                                );
                            var r2;
                        }
                        function d(n3) {
                                    ...
                                    ? (function (n4, t3) {
                                          if (
                                              (function () {
                                                  ...
                                                  return (
                                                      (e._supportsSet = n5), n5
                                                  );
                                              })() &&
                                              Array.from
                                          ) {
                                              for (
                                                  var r3 = new Set(n4), u2 = 0;
                                                  u2 < t3.length;
                                                  u2++
                                              )
                                                  r3.add(t3[u2]);
                        ...
                        function S(n3, t2) {
                            ...
                            n3 in j || (j[n3] = {});
                            for (
                                var r2 = j[n3], e2 = 0, u2 = 0, o2 = 0, i2 = t2;
                                i2 >= 0;

                            ) {
                                if (i2 in r2) {
                                    (e2 = r2[i2].line),
                                        0 === o2 && (o2 = r2[i2].lineStart);
                                    break;
                                }
                                ("\n" === n3.charAt(i2) ||
                                    ("\r" === n3.charAt(i2) &&
                            ...
                            var a2 = e2 + u2,
                                f2 = t2 - o2;
                            return (
                                (r2[t2] = { line: a2, lineStart: o2 }),
                                { offset: t2, line: a2 + 1, column: f2 + 1 }
                            );
                        }
                        function _(n3) {
                        ...
                        function W(n3, t2, r2, e2) {
                            return {
                                from: n3 - t2 > 0 ? n3 - t2 : 0,
                                to: n3 + r2 > e2 ? e2 : n3 + r2,
                            };
                        }
                        function D(n3, t2) {
                            var r2,
                            ...
                            if (w(n3)) {
                                ...
                                (o2 = (function (n4) {
                                    ...
                                    (f2 = (
                                        8 * (o2.to > 0 ? o2.to - 1 : o2.to)
                                    ).toString(16).length) < 2 && (f2 = 2);
                            } else {
                                var g2 = n3.split(/\r\n|[\n\r\u2028\u2029]/);
                                (r2 = c2.column - 1),
                                    (e2 = c2.line - 1),
                        ...
                        function Z(n3) {
                            ...
                            throw new Error(
                                "not a string, regexp, or parser: " + n3,
                            );
                        }
                        function $(n3) {
                            return (
                        ...
                        (u.parse = function (n3) {
                            ...
                            (u.chain = function (n3) {
                                ...
                                return e(function (r2, e2) {
                                    var u2 = t2._(r2, e2);
                                    return u2.status
                                        ? B(n3(u2.value)._(r2, u2.index), u2)
                                        : u2;
                                });
                            }),
                            (u.concat = u.or),
        ...
        var HEADER_CANONICALIZER = parsimmon_umd_min.exports
            .alt(
                parsimmon_umd_min.exports.regex(new RegExp(emojiRegex(), "")),
        ...
        (function (Values2) {
                ...
                switch (wrapped.type) {
                    ...
                    case "date":
                        if (
                            ...
                            return wrapped.value.toFormat(
                                setting.defaultDateFormat,
                            );
                        }
                        return wrapped.value.toFormat(
                            setting.defaultDateTimeFormat,
                        );
                    case "duration":
            ...
            function compareValue(val1, val2, linkNormalizer) {
                ...
                switch (wrap1.type) {
                    ...
                    case "link":
                        ...
                        return (
                            (_a = link1.subpath) !== null && _a !== void 0
                                ? _a
                                : ""
                        ).localeCompare(
                            (_b = link2.subpath) !== null && _b !== void 0
                                ? _b
                                : "",
                        );
                    case "date":
            ...
            function typeOf(val) {
                var _a;
                return (_a = wrapValue(val)) === null || _a === void 0
                    ? void 0
                    : _a.type;
            }
        ...
        (function (Groupings2) {
            ...
            function isGrouping(entry) {
                for (let element2 of entry)
                    if (!isElementGroup(element2)) return false;
                return true;
            }
        ...
        (function (Fields2) {
            ...
            function indexVariable(name) {
                ...
                for (let index2 = 1; index2 < parts.length; index2++) {
                    result = Fields2.index(
                        result,
                        Fields2.literal(parts[index2]),
                    );
                }
                return result;
            }
        ...
        var EXPRESSION = parsimmon_umd_min.exports.createLanguage({
                ...
                parsimmon_umd_min.exports
                    ...
                    .then(
                        parsimmon_umd_min.exports
                            .alt(
                                q.escapeCharacter,
                                parsimmon_umd_min.exports.noneOf('"\\'),
                            )
                            .atLeast(0)
            ...
            identifier: (_) =>
                parsimmon_umd_min.exports
                    .seqMap(
                        ...
                        parsimmon_umd_min.exports
                            .alt(
                                parsimmon_umd_min.exports.regexp(
                                    /[0-9\p{Letter}_-]/u,
                                ),
                                parsimmon_umd_min.exports
                                    .regexp(EMOJI_REGEX)
            ...
            datePlus: (q) =>
                parsimmon_umd_min.exports
                    ...
                    .desc(
                        "date in format YYYY-MM[-DDTHH-MM-SS.MS] or in shorthand",
                    ),
            // A duration of time.
            durationType: (_) =>
                parsimmon_umd_min.exports.alt(
            ...
            variableField: (q) =>
                q.identifier
                    ...
                    .desc("variable"),
            numberField: (q) =>
                q.number.map((val) => Fields.literal(val)).desc("number"),
            stringField: (q) =>
                q.string.map((val) => Fields.literal(val)).desc("string"),
            boolField: (q) =>
                q.bool.map((val) => Fields.literal(val)).desc("boolean"),
            dateField: (q) =>
                parsimmon_umd_min.exports
                    .seqMap(
        ...
        var QUERY_LANGUAGE = parsimmon_umd_min.exports.createLanguage({
            ...
            namedField: (q) =>
                parsimmon_umd_min.exports.alt(
                    ...
                    captureRaw(EXPRESSION.field).map(([value, text2]) =>
                        QueryFields.named(stripNewlines(text2), value),
                    ),
                ),
            sortField: (q) =>
                parsimmon_umd_min.exports.seqMap(
                    EXPRESSION.field.skip(
                        parsimmon_umd_min.exports.optWhitespace,
                    ),
                    parsimmon_umd_min.exports
                ...
                parsimmon_umd_min.exports.seqMap(
                    ...
                    (header, from, clauses) => {
                        return {
                            header,
                            source:
                                from.length == 0 ? Sources.folder("") : from[0],
                            operations: clauses,
        ...
        var getAPI2 = (app) => {
            var _a;
            if (app)
                return (_a = app.plugins.plugins.dataview) === null ||
                    _a === void 0
                    ? void 0
                    : _a.api;
            else return window.DataviewAPI;
        };
        var isPluginEnabled = (app) =>
...
function add_css(target) {
    append_styles(
        target,
        "svelte-1wv2i3",
        "form.svelte-1wv2i3{display:flex;flex-direction:column;gap:1em}button.svelte-1wv2i3{max-width:max-content}",
    );
}
...
function create_if_block(ctx) {
    ...
    return {
        ...
        p(ctx2, dirty) {
            ...
            if (dirty /*selectedQuery, queries*/ & 3) {
                select_option(
                    select,
                    /*selectedQuery*/
                    ctx2[1],
...
function create_each_block(ctx) {
    ...
    return {
        c() {
            ...
            t = text(t_value);
            option.__value = option_value_value = /*query*/ ctx[5];
        ...
        p(ctx2, dirty) {
            if (
                dirty /*queries*/ & 1 &&
                t_value !== (t_value = /*query*/ ctx2[5].name + "")
            )
                set_data(t, t_value);
            if (
                dirty /*queries*/ & 1 &&
                option_value_value !== (option_value_value = /*query*/ ctx2[5])
            ) {
...
function flattenFiles(abstractFiles) {
    return abstractFiles
        .map((abstractFile) => flattenFile(abstractFile))
        .flat();
}
...
var Search = class {
    constructor(plugin) {
        this.searchFiles = async (files, disabledDisabledPaths) => {
            const result = files.filter(
                (file) => !this.isInDisabledFolder(file, disabledDisabledPaths),
            );
            if (result.length <= 0) {
    ...
    async search(query) {
        ...
        if (query.type === "Dataview") {
            ...
            const dataviewResult = await (api == null
                ? void 0
                : api.query(query.query));
            if (
                !dataviewResult ||
                !(dataviewResult == null ? void 0 : dataviewResult.successful)
            ) {
                new import_obsidian3.Notice(
            ...
            result = dataviewResult.value.values
                ...
                .filter(
                    (file) =>
                        file !== null && file instanceof import_obsidian3.TFile,
                )
                .map((file) => file);
        } else {
    ...
    checkTagsWithFile(tags, file) {
        ...
        const fileCacheTags =
            (_a = fileCache == null ? void 0 : fileCache.tags) != null
                ? _a
                : [];
        let frontmatterTags =
            (_c =
                (_b = fileCache == null ? void 0 : fileCache.frontmatter) ==
                null
                    ? void 0
                    : _b.tags) != null
                ? _c
        ...
        const frontmatterTag =
            (_d = fileCache == null ? void 0 : fileCache.frontmatter) == null
                ? void 0
                : _d.tag;
    ...
    isInDisabledFolder(file, disabledDisabledPaths) {
        ...
        return this.plugin.settings.disabledFolders
            ...
            .some((disabledFolder) => {
                ...
                if (
                    disabledDisabledPaths &&
                    trimmedFolder === disabledDisabledPaths
                )
                    return false;
...
function create_fragment2(ctx) {
    ...
    return {
        ...
        p(ctx2, [dirty]) {
            if (dirty /*raw*/ & 1) g.innerHTML = /*raw*/ ctx2[0];
        },
        i: noop,
        o: noop,
        d(detaching) {
                ...
                (svg_data = get_spread_update(svg_levels, [
                    ...
                    (!current ||
                        (dirty /*className*/ & 1 &&
                            svg_class_value !==
                                (svg_class_value =
                                    "fa-icon " /*className*/ + ctx2[0]))) && {
                        class: svg_class_value,
                    },
                    (!current || dirty /*width*/ & 2) && {
                        width:
                            /*width*/
                            ctx2[1],
                    },
                    (!current || dirty /*height*/ & 4) && {
                        height:
                            /*height*/
                            ctx2[2],
                    },
                    (!current || dirty /*label*/ & 512) && {
                        "aria-label":
                            /*label*/
                            ctx2[9],
                    },
                    ...
                    (!current || dirty /*style*/ & 256) && {
                        style:
                            /*style*/
                            ctx2[8],
                    },
                    dirty /*$$restProps*/ & 1024 /*$$restProps*/ && ctx2[10],
                ])),
            );
            toggle_class(
                svg,
...
function create_each_block_1(ctx) {
    ...
    return {
        ...
        p(ctx2, dirty) {
            set_svg_attributes(
                path_1,
                (path_data = get_spread_update(path_1_levels, [
                    dirty /*iconData*/ & 64 /*path*/ && ctx2[27],
                ])),
            );
        },
        d(detaching) {
...
function create_each_block2(ctx) {
    ...
    return {
        ...
        p(ctx2, dirty) {
            set_svg_attributes(
                polygon_1,
                (polygon_data = get_spread_update(polygon_1_levels, [
                    dirty /*iconData*/ & 64 /*polygon*/ && ctx2[24],
                ])),
            );
        },
        d(detaching) {
    ...
    return {
        ...
        p(ctx2, dirty) {
            ...
            if (!updating_data && dirty /*iconData*/ & 64) {
                updating_data = true;
                raw_changes.data = /*iconData*/ ctx2[6];
...
function create_fragment4(ctx) {
    ...
    return {
        ...
        p(ctx2, [dirty]) {
            const svg_changes =
                dirty /*label, width, height, box, combinedStyle, spin, flip, inverse, pulse, className, $$restProps*/ &
                4031
...
function create_fragment5(ctx) {
    ...
    return {
        ...
        p(ctx2, [dirty]) {
            if (
                (!current || dirty /*query*/ & 1) &&
                t0_value !== (t0_value = /*query*/ ctx2[0].name + "")
            )
                set_data(t0, t0_value);
            if (!current || dirty /*query*/ & 1) {
                toggle_class(
                    button1,
                    "active",
                    /*query*/
...
function add_css4(target) {
    append_styles(
        target,
        "svelte-1o6bs82",
        ".query-list.svelte-1o6bs82{display:flex;flex-direction:column;gap:0.25rem}",
    );
}
...
function create_each_block3(ctx) {
    ...
    if (
        ...
        queryitem_props.query = /*query*/ ctx[7];
    }
    queryitem = new QueryItem_default({ props: queryitem_props });
    binding_callbacks.push(() =>
        bind(queryitem, "query", queryitem_query_binding),
    );
    queryitem.$on(
        "editQuery",
        /*editQuery_handler*/
    ...
    return {
        ...
        p(new_ctx, dirty) {
            ...
            if (!updating_query && dirty /*queries*/ & 1) {
                updating_query = true;
                queryitem_changes.query = /*query*/ ctx[7];
...
function create_fragment6(ctx) {
    ...
    let each_blocks = [];
    for (let i = 0; i < each_value.length; i += 1) {
        each_blocks[i] = create_each_block3(
            get_each_context3(ctx, each_value, i),
        );
    }
    const out = (i) =>
...
function add_css5(target) {
    append_styles(
        target,
        "svelte-1u40o4f",
        ".title.svelte-1u40o4f{text-align:center}.title__input.svelte-1u40o4f{text-align:center;background:transparent;padding:0;border:none;font-size:var(--h2-size);width:100%}.content.svelte-1u40o4f{display:flex;flex-direction:column;gap:1em}",
    );
}
...
function create_fragment7(ctx) {
    ...
    let each_blocks_1 = [];
    for (let i = 0; i < each_value_1.length; i += 1) {
        each_blocks_1[i] = create_each_block_12(
    ...
    let each_blocks = [];
    for (let i = 0; i < each_value.length; i += 1) {
        each_blocks[i] = create_each_block4(
            get_each_context4(ctx, each_value, i),
        );
    }
    return {
        c() {
        ...
        m(target, anchor) {
            ...
            append(div4, div3);
            append(div3, input1);
            input1.checked = /*query*/ ctx[0].createCommand;
            append(div26, t5);
            append(div26, div11);
            append(div11, div8);
            append(div11, t9);
            ...
            append(div10, div9);
            append(div9, input2);
            input2.checked = /*query*/ ctx[0].useDisabledFolders;
            append(div26, t10);
            append(div26, div16);
            append(div16, div14);
            append(div16, t14);
        ...
        p(ctx2, [dirty]) {
            if (
                dirty /*query, QUERY_OPEN_TYPES*/ & 1 &&
                input0.value /*query*/ !== ctx2[0].name
            ) {
                set_input_value(
                    input0,
                    /*query*/
            ...
            if (dirty /*query, QUERY_OPEN_TYPES*/ & 1) {
                input1.checked = /*query*/ ctx2[0].createCommand;
            }
            if (dirty /*query*/ & 1) {
                toggle_class(
                    div3,
            ...
            if (dirty /*query, QUERY_OPEN_TYPES*/ & 1) {
                input2.checked = /*query*/ ctx2[0].useDisabledFolders;
            }
            if (dirty /*query*/ & 1) {
                toggle_class(
                    div9,
            ...
            if (dirty /*QUERY_OPEN_TYPES*/ & 0) {
                each_value_1 = ensure_array_like(QUERY_OPEN_TYPES);
                let i;
                for (i = 0; i < each_value_1.length; i += 1) {
                    const child_ctx = get_each_context_12(
                        ctx2,
                        each_value_1,
                        i,
                    );
            ...
            if (dirty /*queryDescription*/ & 4)
                set_data(
                    t23,
                    /*queryDescription*/
                    ctx2[2],
                );
            if (dirty /*queryPlaceholder*/ & 2) {
                attr(
                    textarea,
                    "placeholder",
                    /*queryPlaceholder*/
...
function add_css6(target) {
    append_styles(
        target,
        "svelte-1wfaisu",
        ".query-list-view.svelte-1wfaisu{display:flex;flex-direction:column;gap:1rem}.query-form.svelte-1wfaisu{display:flex;justify-content:right;gap:var(--size-4-2)}",
    );
}
function create_fragment8(ctx) {
    ...
    if (
        ...
        querylist_props.queries = /*queries*/ ctx[0];
    }
    querylist = new QueryList_default({ props: querylist_props });
    binding_callbacks.push(() =>
        bind(querylist, "queries", querylist_queries_binding),
    );
    querylist.$on(
        "deleteQuery",
        /*deleteQuery*/
    ...
    return {
        ...
        p(ctx2, [dirty]) {
            ...
            if (!updating_queries && dirty /*queries*/ & 1) {
                updating_queries = true;
                querylist_changes.queries = /*queries*/ ctx2[0];
                add_flush_callback(() => (updating_queries = false));
            }
            querylist.$set(querylist_changes);
            if (
                dirty /*queryFormName*/ & 2 &&
                input.value /*queryFormName*/ !== ctx2[1]
            ) {
                set_input_value(
                    input,
                    /*queryFormName*/
...
function instance8($$self, $$props, $$invalidate) {
    ...
    const editQuery = (e) => {
        const query = e.detail.query;
        const modal = new EditQueryModal2(plugin.app, query, (query2) => {
...
var QueryView = class extends SvelteComponent {
    constructor(options) {
        ...
        init(
            ...
            create_fragment8,
            safe_not_equal,
            { plugin: 8, queries: 0, saveQueries: 9 },
            add_css6,
        );
    }
};
    ...
    async onload() {
        ...
        this.registerEvent(
            this.app.workspace.on("file-menu", (menu, file) => {
                if (
                    !(file instanceof import_obsidian6.TFolder) ||
                    file.isRoot()
                )
                    return;
                menu.addItem((item) => {
